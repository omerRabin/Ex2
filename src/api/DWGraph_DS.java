...
public class DWGraph_DS implements directed_weighted_graph{
    ...
    public void connect(int src, int dest, double w) {
        ...
        if(!n1.getNi().containsKey(dest)) {
            n1.getNi().put(dest, n2);
            n1.getEdges().put(dest, e);
            this.MC++;
            this.edges++;
        }
    }
        ...
        if(removalNode!=null) {
            ...
            while (it.hasNext()) {
                ...
                if (current.getNi().containsKey(key)) {
                    current.getNi().remove(key, removalNode);
                    current.getEdges().remove(key, removalNode);
                    this.edges--;
                }
            }
            this.MC++;
    ...
    public edge_data removeEdge(int src, int dest) {
        ...
        if(n.getNi().containsKey(dest)){
            n.getNi().remove(dest,getNode(dest));
            n.getEdges().remove(dest,getNode(dest));
            return n.getEdge(dest);
        }
        return null;
    }
